...
async function execute({ sock, msg, args }) {
    ...
    try {
        ...
        // بناء قائمة الفئات (مع نقطة قبل كلمة قائمة وأرقام دائرية)
        let categoriesList = "";
...
> ⏤͟͟͞͞ ~ 𝑲𝒓𝒐𝒍𝒍𝒐 - 𝑩𝛩𝑻 🕸⃝⃕`;
        ...
        // مسار الصورة
const imagePath = path.join(process.cwd(), "accounts/Maky/nova/killua.jpg");
...
> ⏤͟͟͞͞ ~ 𝑲𝒓𝒐𝒍𝒍𝒐 - 𝑩𝛩𝑻 🕸⃝⃕`;
        ...
        const listener = async ({ messages }) => {
            ...
            // إزالة النقطة من البداية إذا وجدت (للدخول على الفئات)
            ...
            if (cleanInput === "رجوع") {
                if (state === "CATEGORY_VIEW") {
                    ...
                    // العودة للقائمة الرئيسية مع إعادة توجيه
                    let newMainMsg;
                    if (fs.existsSync(imagePath)) {
                        newMainMsg = await sock.sendMessage(chatId, {
                            image: fs.readFileSync(imagePath),
                            caption: mainMenuText,
                            mentions: [sender],
                            contextInfo: contextInfoWithChannel
